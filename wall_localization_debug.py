...
def localize(lidar_data, step_size, odometry_data, robotX, robotY, robotTheta, P):
    ...
    print('SL, SR', SL, SR)
    #walls = []
    robotX, robotY, robotTheta, P, corresponded_walls, walls,_= wall_ekf_debug.kalman_filter([robotX, robotY, robotTheta], lidar_data, P, SL, SR)
...
# =============================================================================
    ...
    robotX, robotY, robotTheta = [1.3,1.5,np.pi/2+0.1]
    ...
    robotX, robotY, robotTheta, P, plot_vars,_ = localize(lidar_data, step_size, odometry_data, robotX, robotY, robotTheta, P)
    X1,robotX, robotY, robotTheta, corr_walls, walls = plot_vars
    
    print('after', robotX, robotY, robotTheta)
    plt.scatter(robotX, robotY, c='g', label ="Robot Pose")
    ...
    lidar_data = process_lidar_data(lidar_data, step_size)
    world_coords = get_world_coords(robotX, robotY, robotTheta, lidar_data)
    plot_world_coords(world_coords)
    print('len corr walls', len(corr_walls))
    ...
    for i, wall in enumerate(walls):
        ...
        y = [p1[1][0], p2[1][0]]
        if i == 0:
            plt.plot(x, y,'black', lw =2, label='Extracted Lines')
        else:
    ...
    plt.scatter(0,0, c='r', label='Lidar Points')
    plt.axis('equal')
